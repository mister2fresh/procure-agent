...
- **(B) Helper + node split, not one fat node.** `_match_line` is the part of the cascade worth naming on its own; lifting it out lets `match_node` read as "open conn, map over lines." Three-line node body, ~70-line helper.
- **(C) Explicit five-tier cascade over data-driven loop.** `[(method, callable), ...]` table iterated until something hits is cute; reading five `if` blocks makes the tier order, the guards, and the `MatchMethod` per tier all locally inspectable. The cascade *is* the domain logic. Verbosity is the feature.
...
## 2026-05-06 — read_file encoding pin and notes-sidecar dropdown filter

**500 in production on text fixtures.** `GET /api/fixtures/<plain-text-fixture>` started 500ing once the source-viewer endpoint was rerouted through `agent.read_file`. Local dev didn't reproduce because the dev shell runs `LANG=C.UTF-8`. Railway's container default is `C` — `Path.read_text()` without an explicit encoding falls back to `locale.getpreferredencoding()`, which under `C` is ASCII, and every fixture in the corpus has a UTF-8 em dash on its first banner line (`SUPPLIER — QUOTE`). UnicodeDecodeError → 500. Pinned `encoding="utf-8"` in `read_file`'s non-docx branch. Verified by running the read under `LANG=C LC_ALL=C` against three fixtures (txt, notes.md, docx); pre-fix the first two failed, post-fix all succeed.

**Why this matters as a pattern, not a one-off.** Any code path that crosses from a UTF-8 dev shell to a non-UTF-8 deploy environment via `read_text()` / `open(path)` / `subprocess` text mode is exposed to the same bug. The fix is mechanical (pin the encoding) but the failure mode is invisible in dev. Worth treating as a "container-locale lemma" to keep in mind whenever new file I/O lands on a hot path.

**Sidecar files leaking into the dropdown.** `/fixtures` lists everything in `data/synthetic_quotes/` whose suffix is in `SOURCE_EXTS`. The per-supplier `*.notes.md` review notes match `.md`, so they appeared in the dropdown next to the actual quotes (e.g. `nutrigrow_NG-26-0418.docx` and `nutrigrow_NG-26-0418.notes.md` adjacent after the rename surfaced the duplication). Filtered on the FE by excluding any filename containing `.notes.` since the API surface was actively being iterated in a parallel session — server-side `list_fixtures` should grow the same exclusion when that work settles, so the API doesn't ship sidecar paths to future consumers (Slack bot, MCP server) that lack the FE filter.

## 2026-05-06 — demo set vs. fixture corpus

**Default-fixture extraction broke after the rename.** Post-rename alphabetical sort surfaced `acme-fasteners_Q-2026-04881.csv` as the dropdown's first entry. Anyone clicking "Run extraction" on the default hit a 500 — Haiku returned `quantity: null` for every line item and `Quote.model_validate_json` rejected the payload (`Decimal input should be an integer, float, string or Decimal object`).

**Not an extraction bug.** The fixture is a tier-priced industrial-fasteners quote; each row is one tier band, with no order-quantity column anywhere in the CSV. Per the fixture's own notes file this is a known unrepresentable shape until v1 either lands a `quote_type` discriminator or relaxes `quantity` to nullable. It was kept in the corpus intentionally as a future-work signal but had no `.expected.json` golden — exactly what marked it as not-yet-scorable.

**Corpus and demo set need separate definitions.** Treating "everything in `data/synthetic_quotes/`" as the demo set worked while the corpus was hand-curated to match what the schema could handle. Stops working the moment a future-work fixture lands without a golden and outranks the working set alphabetically. The presence of an `.expected.json` golden is the existing canonical "this shape is supported" marker — no need to invent a new flag.

**Filter at the API.** `list_fixtures` now returns only fixtures with a paired `.expected.json`. Matches the eval set exactly — 14 fixtures of 15 source files. Drops the FE's `.notes.` exclusion as a side effect (sidecar `.notes.md` files don't have goldens). Future fixtures intentionally added without a golden will never pollute the demo dropdown but stay available to scripts that read the directory directly.

**Production-only `/products/search` 500 — root cause not yet known.** Override-picker endpoint 500s in production for every query (`q=`, `q=al`, `q=sde`, `q=sdef` — all 500, ~100-300ms each). Local reproduction is clean: same DATABASE_URL shape, migrations applied, psycopg/Pydantic versions per `uv.lock` — `curl 'http://localhost:8765/products/search?q=&limit=20'` returns 200 with 20 product rows and Decimals serialized as strings. No `/products/search` traceback found in Railway logs around the 500s. Wrapped the endpoint in `try/except Exception → HTTPException(500, detail=f"{type(e).__name__}: {e}")` so the next 500's response body carries the real exception name and message — unblocks the next debug pass without log access.

**Working theories, in priority order until the wrapped error surfaces.**
1. Pydantic v2 Decimal serialization difference between the api Dockerfile's runtime and local. Local serializes to JSON strings (verified by curl); prod might fall through to a serializer that raises on the same input.
2. The `connect()` per-request open against Railway-Postgres exhausts a per-IP/per-account connection cap on rapid debounced typeahead calls. Less likely on a single user demo since even the first request 500s, but the failure pattern doesn't fully exclude it.
3. Migrations partially applied in prod — `0003_pg_trgm.sql` requires `CREATE EXTENSION` permission Railway's managed Postgres role might not grant; `migrate.py` would have errored, `bootstrap_prod_db.py` would have returned non-zero, `preDeployCommand` failed without blocking start. Existing match-node code uses `similarity()` and works in prod, so this is unlikely — included for completeness.

The wrapped-detail response body will discriminate immediately on next test.

## 2026-05-06 — Next.js rewrite proxy is IPv6-blind on Railway

**Single root cause for two production 500s.** Browser-side `/api/fixtures/<name>` returned 500 for every fixture except the SSR-rendered default; `/api/products/search` returned 500 for every query. Direct curl against `https://web.../api/health` also 500s. Server-side rendering through `lib/api.ts` works against the same `API_INTERNAL_URL`. Diagnosis: Next.js 16's wildcard rewrite (`/api/:path* → ${apiBaseUrl}/:path*`) hands proxying off to the bundled `http-proxy` package, which uses Node's built-in `http.request` + `dns.lookup`. Railway's private network is IPv6-only (`api.railway.internal` has only AAAA records), and that DNS path doesn't reliably resolve a v6-only host. Undici (`fetch`) does resolve it — which is why every server component using `fetch` works and every browser path through the rewrite fails. The "Pydantic Decimal serialization" / "connection cap" theories from the previous entry are both ruled out; neither endpoint's Python ever ran.

**Fix: replace the rewrite with two Route Handlers.** `app/api/fixtures/[filename]/route.ts` and `app/api/products/search/route.ts` — each is a thin pass-through that calls `fetch(${apiBaseUrl}/...)` (undici, IPv6-clean) and forwards the upstream status + body + content-type. Removed the `rewrites()` block from `next.config.ts` entirely. SSR was already going through `lib/api.ts` directly to `apiBaseUrl`, so it's unchanged. Two browser-exposed paths now have explicit handlers; future browser-side endpoints add a route file rather than expanding a wildcard.

**Trade-off considered.** Could have shipped `NODE_OPTIONS=--dns-result-order=verbatim` (or `=ipv6first`) on the web service instead — single env var vs. two route files. Rejected because it depends on `http-proxy`'s lookup respecting the order hint and on the OS resolver returning AAAA records first; the route-handler path is deterministic and doesn't rely on undocumented Node DNS internals. Cost is two ~12-line shims per browser-fetched endpoint, which is small enough that "explicit handler per path" is the cleaner default going forward anyway.

**Container-locale lemma extends.** Yesterday: dev's `LANG=C.UTF-8` masked a `read_text()` encoding bug that surfaced under Railway's `C` locale. Today: dev's localhost-IPv4 networking masked a Node DNS-resolution gap that only appears under Railway's IPv6-only private DNS. Both are dev-deploy-environment skew bugs invisible to local smoke tests. Worth treating "things that differ between dev and Railway" — locale, DNS, filesystem case-sensitivity, time zone — as a checklist whenever something works locally and 500s in prod with no obvious diff in the application code.

**Confirmed in prod.** Override picker on the deployed web service lists inventory and narrows on type. Closes both `/products/search` and `/api/fixtures/<name>` 500s; the IPv6/route-handler diagnosis was the full root cause — wrapped-detail debug shim was never needed.

## 2026-05-06 — agent_runs wiring

**Tracing lives in the API handlers, not in a graph node.** Two paths were on the table: a `persist_node` after `approval` (every invoker traced for free) vs. inserts/updates from `start_run` and `resume_run`. The "free tracing" advantage of the node path collapses on inspection — the only non-FastAPI invoker is `graph.py:__main__`, a dev affordance no real user hits, and the eval harness deliberately shouldn't pollute `agent_runs` with hundreds of CI rows. Meanwhile, writing rows on graph exceptions (status=`failed`) is naturally a request-cycle concern: a node can't catch a *prior* node's exception without a callback handler. Handler path is half the code and covers 100% of the surface that matters.

**Status taxonomy.** `running` / `pending_approval` / `completed` / `failed`. `start_run` inserts at `running`, transitions to `pending_approval` after the graph hits the interrupt, or `failed` on graph exception. `resume_run` transitions to `completed` (stamping `completed_at`) on success, or `failed` on exception. Validation failures from the resume endpoint (404/409/422) leave the row untouched — they're request-validation failures, not graph state changes; the run is still legitimately paused at `approval`. No new "decision_invalid" status needed; the absence of a row update is the correct signal.

**`thread_id` promoted to a real column.** Resume needs to find the row from `thread_id`, the public correlator the API already uses (`/runs/{thread_id}`). Two options: add a column, or `WHERE final_state->>'thread_id' = %s` against the JSONB blob. JSONB-key lookups are slower, indexable only with a functional index, and stylistically odd for what's effectively a foreign key to the LangGraph checkpoint. Migration `0004_agent_runs_thread_id.sql` adds `thread_id text` plus a btree index. `get_agent_run_id_by_thread_id` resolves `ORDER BY started_at DESC LIMIT 1` so a reused `thread_id` (a caller could pass the same one twice) deterministically picks "the live run we'd resume."

**`final_state` JSONB shape.** Mirrors `_to_snapshot` minus `matched_products` (catalog-side data, cheaply re-fetchable from `products`) and minus `messages` (full LLM trace, bloats fast and isn't needed for replay). `thread_id` is stashed inside the JSONB as well — redundant with the column today, but it makes the blob self-describing if it's ever exported on its own. Pydantic models dumped in `mode="json"` so Decimals/dates/UUIDs serialize as JSON-native scalars (matches what FastAPI emits over the wire, so the same shape round-trips).

**`final_state` is null on failure.** Partial graph state on exception is rarely coherent — extract may have raised before producing a Quote, or `_flag_one` may have left `match.flags` half-mutated. The status row alone is sufficient to surface the failure for the README's traced-runs section; structured failure detail (exception type, traceback, last-completed node) can be added if debugging-against-the-row becomes a recurring need. Deliberate v1 trade-off, not a bug.

**`run_id` not exposed to the FE.** The surrogate UUID is server-side only — no trace page exists, no admin UI consumes it, and YAGNI says don't surface a field nobody uses. Cost to expose later is one line on `RunSnapshot` and one in `_to_snapshot`. `thread_id` remains the public identifier; agent_runs.id is the database row's identity, not the run's.

**`quote_id` and `langsmith_run_id` stay nullable.** Backlog item 7 in the handoff captures the (b) write-through path for `quotes` + `quote_line_items` + `agent_runs.quote_id`, gated on the product needing analytical queries (cross-supplier history, duplicate-quote detection across reissues). LangSmith run-id capture deferred — it requires either reading from the Anthropic response metadata or hooking the LangSmith tracer callback, neither of which is on the critical path for the README.

**Two policy helpers, both ≤10 lines.** `_record_run_state(run_id, thread_id, values, status)` writes the snapshot + status; `_mark_run_failed(run_id)` writes the failed terminal without a payload. Both no-op when `run_id is None`, so older runs that pre-date this wiring don't 500 on resume. Keeps `start_run` and `resume_run` under the 40-line cap.

**End-to-end smoke against `rootwise_RW-2026-0419.txt`.** `POST /runs` → row inserted, transitioned to `pending_approval`, `final_state` carries the extracted quote + matches with both `price_variance` and `pack_size_drift` flags as expected. `POST /runs/{thread_id}/resume` with `approve` → status flips to `completed`, `completed_at` stamped, `final_state.human_decision.reviewer` round-trips through the JSONB unchanged. 83/83 unit tests still green; lint clean.

## 2026-05-06 — SKU-typo fixture

**Cascade tiers 3–4 were unit-test-only.** `find_products_by_sku_similarity` had unit coverage and the override picker had its own UI test, but no fixture in the demo corpus drove a quote line into `SUPPLIER_SKU_FUZZY` end-to-end. The demo dropdown could show the picker UI only on UNMATCHED rows and on the nutrigrow substitution case — neither exercises the "trigram returns a candidate, human confirms or replaces it" path that's the picker's reason for existing.

**`terragreen_TG-2026-1505.csv` — five typos, one supplier.** TerraGreen is already the preferred supplier for FEM-50 / BLDM-50 / ALFM-50 / GREENS-CC / BIOCH-2CF in the seeded inventory, so a single quote covering all five inventory entries is supplier-realistic. Every line carries a deliberate SKU mistype: transposed letters (`FME-50`, `BLMD-50`), missing dash (`ALFM50`), dropped letter (`GRENS-CC`), extra trailing letter (`BIOCH-2CFT`). Five distinct typo patterns, all close enough to clear the trigram threshold against *something* in the catalog — the question per line is whether trigram lands on the right neighbor.

**The wrong-sibling case is the actual point.** First run: lines 1, 2, 4 fuzzy-match the intended SKU (human approves). Line 3 (`GRENS-CC` → `GREENS-CC`) matches correctly and additionally surfaces a `pack_size_drift` flag — bonus UX coverage. Line 0 (`FME-50`) matches `FBM-50` at 0.40 — the *wrong* sibling — because `FBM-50` (Pacific Agri's feather meal) and `FEM-50` (TerraGreen's feather meal) are both one edit from the typo, and trigram tiebreak picks alphabetically. This is exactly the scenario the override picker was built for: the model returns a confident-looking match that's wrong on supplier-knowledge grounds, and the human swaps to the right SKU. A fixture that *only* exercised correct fuzzy-recovery would understate what the picker is doing.

**Comparator clean against the golden.** 59/59 fields match `terragreen_TG-2026-1505.expected.json`. The golden captures every typo verbatim on `requested_sku` / `supplier_sku` per the extract-verbatim discipline; pack_size derivation fires correctly on the four lines where description carries a size span and uom is a packaging noun, stays null on the GREENS-CC line where uom is the canonical `lb` weight unit. 83/83 unit tests still green; ruff clean.

**Demo set picks it up automatically.** `list_fixtures` returns sources with a paired `.expected.json`; the new fixture has both. No FE or API change needed — the dropdown surfaces it on the next request, and the eval harness now scores against 15 fixtures instead of 14. Demo invariant from yesterday's entry holds: the future-work corpus members (acme-fasteners) stay invisible while real coverage grows.

## 2026-05-06 — Traced sample runs in the README

**Three fixtures, four share-links.** Section sits between *Failure mode* and *Local development*. Each fixture is named to a specific architectural claim — happy-path baseline (`aloe-corp_AC-2026-0421`), trace-separability (`cascade-bio_CBS-2026-0428-A`, every line clean-matches and the work is in `flag`), fuzzy-cascade + override picker (`terragreen_TG-2026-1505`). Click-through evidence for prose claims, not standalone artifacts.

**Two-phase HITL = two LangSmith root runs per `thread_id`.** `interrupt_before=["approval"]` ends one `g.invoke()` and `resume_run` starts another. PostgresSaver stitches the *state* via `thread_id`; LangSmith treats each invoke as its own top-level trace, and share URLs are per-run. Section intro names this so a reader doesn't expect one URL per fixture.

**Pre-approval-only on aloe and cascade-bio; both halves on terragreen.** Resume on the first two is just `finalize` (state-projection node, no LLM call) — linking it would dilute the page without adding architectural coverage. On terragreen the resume invoke is where the override `LineDecision` arrives in the input state and `_flag_one` re-runs against the override product; that's the HITL-is-real evidence and earns its own link. Considered the alternative of recording all three under a single parent span via a `@traceable` demo script (one URL per fixture, clean), rejected — the demo path would have diverged from the production HITL flow, and a reviewer who notices that gets the wrong signal.

**`agent_runs.langsmith_run_id` still null.** Capture path (collect_runs callback around the `g.invoke` calls in `api.py`, or post-hoc `Client.share_run` from a script) deliberately deferred — the README permalinks were assembled from the LangSmith UI, not from the column. Wiring it earns a row-by-trace audit trail in the DB but unlocks nothing visible to the README reader. Stays in the backlog at the same priority as before.

**Public-share caveat.** `share_run` URLs strip org info but trace contents (prompts, fixture filenames, supplier names, model responses) are public. Procure-agent's corpus is synthetic so this is fine; `Client.unshare_run(run_id)` revokes a link if one ever needs to be pulled.
