...
- **Currency**: ISO code only — emit `"USD"`, `"EUR"`, `"CAD"`, etc., or `null`. **The default is `null`.** Populate ONLY when the source contains an explicit ISO-style currency annotation: a column header like "Unit Price (USD)", a footer like "All prices in USD", a subtotal stated as "USD 2,691.60", or an inline phrase naming the currency. Bare symbols (`$`, `€`) are insufficient — `$` alone → `null`. **Never infer currency from supplier address, area code, state name, ZIP code, or country of operation** — a US-based supplier with `$` prices and no ISO code emits `currency: null` on every line. Same for `€`-with-no-EUR, `£`-with-no-GBP. This rule is absolute; do not override it based on context plausibility.
