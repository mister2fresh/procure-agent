...
# Field-specific
- **customer_ref**: the supplier's persistent identifier for the buyer in their system — Customer #, Account #, customer code, Bill-To ID. Same value across every quote that supplier sends. Capture verbatim. **Names are not IDs**: when the source shows only the buyer's company name (e.g., "Bill To: <Buyer Name Inc.>") with no alphanumeric account code alongside, emit `null` — customer names are not captured anywhere in this schema. Do **not** confuse with per-transaction refs (RFQ-####, PO #, "Buyer Ref:", "Your Ref:") — those go in `rfq_ref`, not here.
